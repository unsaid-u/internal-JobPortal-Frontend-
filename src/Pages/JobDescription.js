...
export default function JobDescription(){
    ...
    const get_job = async()=> {
        const response = await fetch(`http://localhost:8000/job_openings/${jobID}`)
        const data = await response.json()
        setJob(data)
    }

	const get_applications = async ()=>{
		const response = await fetch(`http://localhost:8000/applications/${jobID}`)
		const data = await response.json()
		//console.log(data)
    ...
    return(
        <Container maxWidth='lg'>
           ...
           <br/>
            ...
            <div style={{whiteSpace : 'pre'}} className='lists size'>
            ...
            <div >
                {showApp && <h2>Applications List</h2>}
                <hr/>
                <br/>
                {    
